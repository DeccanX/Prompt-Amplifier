...
% Standard packages (no external dependencies)
...
\title{PRIME: Prompt Refinement via Information-driven Methods and Expansion---A Modular Framework for Context-Aware Prompt Amplification}

\author{
Rajesh More$^{1}$, Madhukar Patneedi$^{1}$, Bhanu Prakash Doppalapudi$^{1}$, \\
Raghav Chaturvedi$^{1}$, David Trakhtenberg$^{1}$, Jyoti Ambad$^{2}$, \\
Sidhant Gupta$^{1}$, Rukesh Patel$^{1}$ \\[1em]
$^{1}$Cloudwerx Worldwide (CWX) \quad $^{2}$Research Scholar \\
\texttt{rajesh.more@cwx.tech}
}
...
%==============================================================================
\begin{abstract}
While Large Language Models (LLMs) have transformed natural language processing, their effectiveness depends critically on prompt quality---a gap that existing work has largely overlooked. Current Retrieval-Augmented Generation (RAG) systems retrieve documents to generate \textit{answers}; we propose a fundamentally different approach: using retrieval to construct \textit{better questions}. This paper introduces \primesys{} (\textbf{P}rompt \textbf{R}efinement via \textbf{I}nformation-driven \textbf{M}ethods and \textbf{E}xpansion), a framework that treats prompt construction as a first-class optimization problem.

\primesys{} implements a modular pipeline with heterogeneous document loaders (10+ formats), pluggable embedding strategies (sparse TF-IDF/BM25 and dense Sentence-BERT/Gemini-Embedding-001/OpenAI), persistent vector stores, and multi-provider LLM generators (GPT-4o, Claude-3, Gemini-2.5-Flash, Gemini-3-Flash-Preview). We formalize prompt amplification mathematically and introduce four novel evaluation metrics: structural coherence ($\mathcal{S}$), semantic specificity ($\mathcal{P}$), contextual completeness ($\mathcal{C}$), and lexical readability ($\mathcal{L}$).

Comprehensive experiments across four domains, 12 embedding configurations, 6 LLM backends, and 30 human-evaluated prompts reveal: (1) dense embeddings achieve 37--73\% higher retrieval precision; (2) Gemini-3-Flash-Preview achieves 165$\times$ expansion ratio; (3) complex queries outperform simple ones by 92\%; (4) human evaluators rate \primesys{} outputs 4.2/5 with 87\% inter-rater agreement; and (5) caching provides 1,944$\times$ speedup. We also present systematic failure analysis identifying when amplification degrades performance. \primesys{} is released as an open-source library (\texttt{pip install prompt-amplifier}).
