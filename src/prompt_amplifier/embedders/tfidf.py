...
class TFIDFEmbedder(BaseSparseEmbedder):
    ...
    def embed(self, texts: Sequence[str]) -> EmbeddingResult:
        ...
        if not self._fitted:
            raise EmbedderError(
                "TFIDFEmbedder must be fitted before embedding. "
                "Call fit() or fit_embed() first."
            )
...
class BM25Embedder(BaseSparseEmbedder):
    ...
    def fit(self, texts: Sequence[str]) -> "BM25Embedder":
        ...
        try:
            ...
            raise ImportError(
                "rank_bm25 is required for BM25Embedder. "
        ...
        self._fitted = True
        self._vocabulary_size = len(set(
            token for tokens in self._corpus_tokens for token in tokens
        ))
