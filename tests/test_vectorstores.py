...
from prompt_amplifier.core.exceptions import VectorStoreError, DocumentNotFoundError
...
class TestMemoryStore:
    ...
    def test_filter_search(self, memory_store, fitted_tfidf_embedder):
        ...
        chunks = [
            Chunk(content="Content A", document_id="doc", chunk_index=0, metadata={"category": "A"}),
            Chunk(content="Content B", document_id="doc", chunk_index=1, metadata={"category": "B"}),
        ]
        fitted_tfidf_embedder.embed_chunks(chunks)
        memory_store.add(chunks)
